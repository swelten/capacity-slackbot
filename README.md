...
- **`/capacity-ping` slash command** – Without arguments it starts the same chat for the command user (handy for ad-hoc edits). Run `/capacity-ping broadcast` to trigger the reminder DM immediately for everyone (mirrors the EventBridge run). Use `/capacity-ping KW 45 [2025]` to open the form for a specific calendar week.
